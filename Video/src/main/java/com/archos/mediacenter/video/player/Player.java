...
public class Player implements IPlayerControl,
    ...
    private HandlerThread wledThread;
    private WLEDHandler wledHandler;
    ...
    public void setEffect(int type, int mode) {
        ...
        if (needToSupportGLEffect ^ mSurfaceController.supportOpenGLVideoEffect())
            setGLSupportEnabled(needToSupportGLEffect);
        if (mEffectRenderer != null) {
    ...
    synchronized public void stopPlayback() {
        ...
        if (wledThread != null) {
            wledHandler.sendEmptyMessage(1);
            wledThread.quitSafely();
        }
    }
    ...
    public void openVideo() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            wledThread = new HandlerThread("wled");
            wledThread.start();
            wledHandler = new WLEDHandler(wledThread.getLooper(), this);
        }
    }
    ...
    public void start(int state) {
        ...
        wledHandler.sendEmptyMessage(0);
    }
    ...
    public static class WLEDHandler extends Handler {
        ...
        public void handleMessage(@NonNull Message msg) {
            ...
            int mSurfaceWidth = player.mSurfaceWidth;
            int mSurfaceHeight = player.mSurfaceHeight;
            SurfaceHolder mSurfaceHolder = player.mSurfaceHolder;
